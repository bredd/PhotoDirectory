...
namespace FileMeta
{
    static class ImageFile
    {
        ...
        /// </remarks>
        public static void ResizeAndRightImage(string srcFilename, string dstFilename, int width, int height)
        {
            if (width == 0 && height == 0) throw new InvalidOperationException("New resolution must be specified.");
            using (var srcStream = new FileStream(srcFilename, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                using (var dstStream = new FileStream(dstFilename, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    ResizeAndRightImage(srcStream, dstStream, width, height);
                }
            }
        }
